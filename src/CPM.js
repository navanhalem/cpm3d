...
class CPM {
	...
	*/
	...
	findClosestStroma ( sourcecoord ) {
		let minDist = 999999
		let goal = sourcecoord
		let stromapix = Object.keys(this.stromapixelstype)
		for ( let i = 0; i < stromapix.length; i++ ) {
			let dist = 0
			for ( let l = 0; l < sourcecoord.length; l ++ ) {
				dist = dist + Math.abs(sourcecoord[l]-stromapix[i][l])
			}
			if ( dist < minDist ) {
				minDist = dist
				goal = stromapix[i]
	...
	deltaHtostroma ( sourcei, targeti, src_type, tgt_type ) {
    let deltaH = 0, stromaattraction
    // use parameters for the source cell, unless that is the background.
    // In that case, use parameters of the target cell.
    if( src_type != 0 ){
      stromaattraction = this.par("LAMBDA_SA",src_type)
    } else {
      // special case: punishment for a copy attempt from background into
      // an active cell. This effectively means that the active cell retracts,
      // which is different from one cell pushing into another (active) cell.
      stromaattraction = this.par("LAMBDA_SA",tgt_type)
    }

		let goal = this.findClosestStroma( this.i2p(sourcei) )

    let p1 = this.i2p(sourcei), p2 = this.i2p(targeti), dir = [goal[0][0]-p1[0]/goal[1],goal[0][1]-p1[1]/goal[1],goal[0][2]-p1[2]/goal[1]], r = 0., norm1 = 0, norm2 = 0, d1 = 0., d2 = 0.
		// loops over the coordinates x,y,(z)
		for( let i = 0; i < p1.length ; i++ ){
			// direction of the copy attempt on this coordinate is from p1 to p2
			d1 = p2[i] - p1[i]
		...
		let ang =  r/Math.sqrt(norm1)/Math.sqrt(norm2)
		deltaH = -stromaattraction * ang
    return deltaH
	}

	deltaHcenter ( sourcei, targeti, src_type, tgt_type ) {
		let deltaH = 0, lambdacenter
		if( src_type != 0 ){
      lambdacenter = this.par("LAMBDA_CENTER",src_type)
    } else {
      // special case: punishment for a copy attempt from background into
      // an active cell. This effectively means that the active cell retracts,
      // which is different from one cell pushing into another (active) cell.
      lambdacenter = this.par("LAMBDA_CENTER",tgt_type)
    }
		let p1 = this.i2p(sourcei), p2 = this.i2p(targeti), pt = [this.field_size.x/2, this.field_size.y/2, this.field_size.z/2], r = 0., norm1 = 0, norm2 = 0, d1 = 0., d2 = 0.
		for( let i=0 ; i < p1.length ; i ++ ){
